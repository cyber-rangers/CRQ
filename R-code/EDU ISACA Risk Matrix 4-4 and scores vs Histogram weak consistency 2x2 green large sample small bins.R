...
lr = c(slp*sli, slp*smi, slp*ssi, slp*shi, smp*sli, ssp*sli, shp*sli, smp*smi)  #low risks defined as green region in risk matrix and its possible variants, R=P*I
mr = c(smp*ssi, smp*shi, ssp*smi, shp*smi)  #med risks defined as yellow region in risk matrix and its possible variants, R=P*I
...
mr3 = c(slp*ssi, ssp*sli)          #med risks with rating 3 marked as yellow region in risk matrix, R=P*I
mr4 = c(smp*smi, slp*shi, shp*sli) #med risks with rating 4 marked as yellow region in risk matrix, R=P*I
...
dsr6 = data.frame(x=sr6)           #data frame with significant risks and label "R6" (rating 6)  
dsr6$r_rating = "R6"

dsr8 = data.frame(x=sr8)           #data frame with significant risks and label "R8" (rating 8)  
dsr8$r_rating = "R8"

dsr9 = data.frame(x=sr9)           #data frame with significant risks and label "R9" (rating 9)  
dsr9$r_rating = "R9"

dhr12 = data.frame(x=hr12)         #data frame with high risks and label "R12" (rating 12)  
dhr12$r_rating = "R12"
...
scale_fill_manual(values = c("#CC0033","#33FF00","#FFFF33","#FFBF00")) +
  xlab(label = "Yc") +
  ylab("Count") +
  ggtitle("Histogram of Cyber Risk 4x4 Weakly Consistent Matrix Colors")
...
ggplot(dfr, aes(x = x, fill = r_rating)) + 
  ...
  xlab("Yc") +
  ylab("Count") +
  ggtitle("Histogram of Cyber Risk Weakly Consistent Colored Ratings")
